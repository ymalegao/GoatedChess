...
void Game::endTurn()
{
	...
	cout << "current player" << getCurrentPlayer()->playerNumber() << endl;
}
...
{
	if (dst.bit() == &bit) {
        cout << "Piece successfully moved to destination BitHolder." << endl;
        endTurn(); // Only end the turn if the move is successful
    } else {
        cout << "Error: Piece was not moved to destination BitHolder." << endl;
    }
...
void Game::updateAI()
{
    while (gameHasAI() && getCurrentPlayer()->isAIPlayer())
	{
		if (gameHasAI() && getCurrentPlayer()->isAIPlayer())
		{
...
void Game::mouseUp(ImVec2 &location, Entity *entity)
{
	...
	if (!_dragBit)
	{
		...
		cout << "No bit was clicked." << endl;

		cout << "mouseUp2" << endl;

		if (entity && entity->getEntityType() == Entity::EntityBitHolder)
		{
			cout << "BitHolder clicked" << endl;
			BitHolder *holder = (BitHolder *)entity;
			cout << "mouseUp3" << endl;

			if (actionForEmptyHolder(*holder))
			{
				cout << "actionForEmptyHolder" << endl;
				_dropTarget = nullptr;
				_dragBit = nullptr;
				return;
			}
	...
	if (_dragBit)
	{
		...
		if (_dragMoved)
		{
			...
			if (_dropTarget && _dropTarget->dropBitAtPoint(_dragBit, _dragBit->getPosition()))
			{
				...
				cout << "Piece successfully dropped into the bit holder." << endl;
				...
				{
					cout << "Piece was not dropped into the bit holder." << endl;
				}
				_dragBit->setPickedUp(false);
				...
				cout <<"draggedBitTo" << endl;
				bitMovedFromTo(*_dragBit, *_oldHolder, *_dropTarget);
				_dropTarget->setBit(_dragBit);
			}
			else
			{
				// Nope, cancel:
